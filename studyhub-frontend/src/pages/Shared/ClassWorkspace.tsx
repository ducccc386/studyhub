...
const ClassWorkspace: React.FC = () => {
  ...
  const handleRequestFinalPayment = async () => {
    if (!window.confirm("Xác nhận đã dạy xong và hoàn thành khóa học này?")) return;
    try {
      ...
      if (res.ok) {
        ...
        alert("Đã báo cáo hoàn thành lớp học!");
      } else {
                ...
                <div className="space-y-4">
                  ...
                  {isTutor ? (
                    ...
                    >
                      ...
                      Xác nhận đã dạy xong (Hoàn thành lớp học)
                    </button>
                  ) : (
                    <p className="text-xs text-on-surface-variant">Phần học phí 75% còn lại và tiền các tháng học tiếp theo sẽ tự thanh toán trực tiếp giữa Phụ huynh và Gia sư.</p>
                  )}
                </div>
              ) : (
                <div className="space-y-4">
                  ...
                  <p className="text-xs text-on-surface-variant text-center">Đã hoàn thành khóa học (Đóng 25% hoa hồng cho hệ thống). Hai bên tự tất toán phần học phí còn lại trực tiếp với nhau.</p>
                </div>
              )}
            </div>
          </div>
